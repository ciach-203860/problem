...
public interface DetectionRepository extends JpaRepository<DetectionMeasurement, Long> {
    ...
    List<DetectionMeasurement> getDetectionMeasurementsFromPeriodOfTime(@Param("start")LocalDateTime start,
                                                                    @Param("end")LocalDateTime end);

    List<DetectionMeasurement> findByDeviceId(Long deviceId);
}
